        ...
        if form.is_valid():
            ...
            for rs in shopcart:
                ...
                if  rs.product.variant=='None':
                    ...
                    variant.save()
                #************ <> *****************
            # message = "Daclyns order info: /n " + " "+ data.full_name + " " +  data.phone + " " +  str(data.total) + " "+ str(ordercode) +" "+ data.address
            # send_mail('Order Form', message , settings.EMAIL_HOST_USER, ['dansokofi25@gmail.com'], fail_silently=False)
            ...
            return render(request, 'Order_Completed.html',{'pk_public': settings.PAYSTACK_PUBLIC_KEY, 'ordercode':ordercode,'category': category, 'email': data.email, 'total': data.total})
